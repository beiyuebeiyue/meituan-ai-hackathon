...
function ActionChip({
  ...
  return (
    ...
    >
      <Text
        style={[styles.actionChipText, { color: textColor }]}
        numberOfLines={1}
      >
        {label}
      </Text>
    ...
    >
      ...
      <View style={styles.recommendationBody}>
        ...
        <View style={styles.recommendationTypeRow}>
          <Text
            style={[styles.recommendationTypeText, { color: colors.subtext }]}
            numberOfLines={1}
          >
            {typeLabel}
          </Text>
        ...
        <View
          style={[
            styles.recommendationReasonBox,
            { backgroundColor: colors.surfaceAlt },
          ]}
        >
          <Text
            style={[styles.recommendationReasonLabel, { color: colors.accent }]}
          >
            推荐理由
          </Text>
          <Text
...
export function AskAIScreen() {
  ...
  >(null);
  const [queuedTryOnStyleId, setQueuedTryOnStyleId] = useState<string | null>(
    null,
  );
  ...
  const [materializingNoteId, setMaterializingNoteId] = useState<string | null>(
    null,
  );
  const [queuedTryOnNoteId, setQueuedTryOnNoteId] = useState<string | null>(
    null,
  );
  ...
  const chatMutation = useMutation({
    mutationFn: async (payload: ChatPayload) => {
      ...
      const requestPayload =
        payload.handImageUri || payload.savedHandPhotoId
          ? {
              ...
              handImageUri: payload.handImageUri
      ...
      try {
        const data =
          payload.handImageUri || payload.savedHandPhotoId
            ? await api.chatWithHand(
                payload.messages,
                payload.handImageUri,
                payload.savedHandPhotoId,
              )
            : await api.chat(payload.messages);
        setChatDebugEntries((entries) =>
          entries.map((entry) =>
            entry.id === debugId ? { ...entry, response: data } : entry,
          ),
        );
        return data;
      } catch (error) {
        ...
        setChatDebugEntries((entries) =>
          entries.map((entry) =>
            entry.id === debugId ? { ...entry, error: message } : entry,
          ),
        );
        throw error;
      }
    ...
    onSuccess: (data, payload) => {
      ...
      setRecommendations(
        data.needs_hand_image ? [] : (data.recommendations ?? []),
      );
      const nextMessages = [
        ...payload.messages,
    ...
    if (intent === "hand_match" && askedQuery) {
      ...
      const nextMessages: AIChatMessage[] =
        lastMessage?.role === "user" && lastMessage.content === askedQuery
          ...
          : [
              ...chatMessages,
              { role: "user" as const, content: askedQuery },
            ].slice(-12);
      setChatMessages(nextMessages);
  ...
  const recentHandPhotos = useMemo(
  ...
  const conversationMessages = useMemo<AIChatMessage[]>(() => {
    ...
    if (
      assistantLine &&
      (lastMessage.role === "user" || lastMessage.content !== assistantLine)
    ) {
      return [
        ...chatMessages,
        { role: "assistant" as const, content: assistantLine },
      ].slice(-13);
    }
    return chatMessages;
  ...
  useEffect(() => {
    ...
    const current = (savedHandsData?.items ?? []).find(
      (item) => item.id === selectedHandPhotoId,
    );
    if (!current) {
      setSelectedHandPhotoId(null);
      return;
  ...
  useEffect(() => {
    if (!token || !queuedTryOnStyleId || !hasHandSelection || isStartingTryOn)
      return;
  ...
  const submitPrompt = (
    nextPrompt?: string,
    options?: { forceHandMatch?: boolean },
  ) => {
  ...
  const startTryOn = (
    styleId: string,
    options?: { forceHandReady?: boolean },
  ) => {
  ...
  const openConversation = (conversation: AskAIConversation) => {
    ...
    setAssistantLine(
      lastAssistant?.content ?? "这轮对话已经打开，你可以继续问我。",
    );
    setRecommendations([]);
    setNeedsHandFor(null);
    setHandPickerMessage("");
    ...
    >
      ...
      >
        ...
        >
          ...
          {handImageUri ? (
            ...
            >
              <View style={styles.currentHandCopy}>
                <Text style={[styles.currentHandTitle, { color: colors.text }]}>
                  当前使用的手图
                </Text>
                <Text
                  style={[styles.currentHandText, { color: colors.subtext }]}
                >
                  {selectedHandPhotoId
                    ? "来自你最近上传的手图"
                    : "来自本次临时上传的手图"}
          ...
          {displayedRecommendations.length ? (
            <View style={styles.resultSection}>
              <View style={styles.resultSectionHeader}>
                <Text
                  style={[styles.resultSectionTitle, { color: colors.text }]}
                >
                  为你推荐
                </Text>
                <Text
          ...
          {tryOnJobQuery.data?.status === "succeeded" &&
          currentResultImageUrl ? (
            <View
              style={[styles.resultCard, { backgroundColor: colors.surface }]}
            >
              <View style={styles.resultCardHeader}>
                <Text style={[styles.resultCardTitle, { color: colors.text }]}>
                  这次的上手效果
                </Text>
                {activeStyleQuery.data ? (
                  ...
                  >
                    <Text
                      style={[
                        styles.nailTypePillText,
                        { color: colors.accent },
                      ]}
                    >
                      {getNailTypeLabel(activeStyleQuery.data.nail_type)}
                    </Text>
                  </View>
                ) : null}
              </View>
            ...
            >
              ...
              <View style={{ flex: 1, gap: 6 }}>
                ...
                <Text style={[styles.errorText, { color: colors.subtext }]}>
                  {tryOnJobQuery.data.error_message ??
                    "可以换张手图，或者换一款美甲再试一次。"}
                </Text>
              </View>
            </View>
          ...
          >
            <TextInput
              style={[
                ...
                {
                  color: colors.text,
                  opacity: chatMutation.isPending ? 0.45 : 1,
                },
              ]}
            ...
            <AnimatedPressable
              ...
              onPress={() =>
                Alert.alert("语音输入即将上线", "这一步我先给你留好了入口。")
              }
            >
        ...
        >
          ...
          >
            <Pressable
              style={[styles.previewPanel, { backgroundColor: colors.surface }]}
              onPress={(event) => event.stopPropagation()}
            >
              {previewItem ? (
                <>
                  <Image
                    source={{ uri: resolveAssetUrl(previewItem.image_url) }}
                    style={[
                      styles.previewImage,
                      { backgroundColor: colors.accentSoft },
                    ]}
        ...
        >
          <View style={styles.historyOverlay}>
            ...
            >
              <ScrollView
                contentContainerStyle={styles.historyDrawerContent}
                showsVerticalScrollIndicator={false}
              >
                <Pressable
                  style={[
                ...
                >
                  ...
                  />
                  <Text
                    style={[styles.newConversationText, { color: colors.text }]}
                  >
                    新建对话
                  </Text>
                </Pressable>
                ...
                <View style={styles.historySkillRow}>
                  ...
                  >
                    ...
                    />
                    <Text
                      style={[styles.historySkillText, { color: colors.text }]}
                    >
                      找热门款
                    </Text>
                  </Pressable>
                ...
                <Text
                  style={[styles.historyEndText, { color: colors.subtext }]}
                >
                  已经到底啦
                </Text>
              </ScrollView>
            ...
            <SafeAreaView style={styles.debugSafeArea}>
              ...
              >
                <View>
                  ...
                  <Text
                    style={[styles.debugSubtitle, { color: colors.subtext }]}
                  >
                    最近 20 次 / 仅本机当前页面
                  </Text>
                </View>
              ...
              >
                {chatDebugEntries.length ? (
                  chatDebugEntries.map((entry) => (
                    ...
                    >
                      <Text
                        style={[styles.debugTime, { color: colors.subtext }]}
                      >
