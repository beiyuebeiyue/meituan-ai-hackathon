...
function markInboxReadLocally(
  payload: MessageInboxResponse | undefined,
): MessageInboxResponse | undefined {
...
export function MessagesInboxScreen({
  ...
  const unreadCount =
    (query.data?.badge.main_unread_count ?? 0) +
    (query.data?.stranger_bucket?.unread_count ?? 0);
  const readAllMutation = useMutation({
    mutationFn: () => api.markAllMessagesRead(),
    onMutate: async () => {
      await queryClient.cancelQueries({ queryKey: ["message-inbox"] });
      const previousInbox = queryClient.getQueriesData<MessageInboxResponse>({
        queryKey: ["message-inbox"],
      });
      queryClient.setQueriesData<MessageInboxResponse>(
        { queryKey: ["message-inbox"] },
        markInboxReadLocally,
      );
          ...
          ListHeaderComponent={
            <View style={styles.headerWrap}>
              ...
              <View style={styles.socialCards}>
                {socialCards.map((item) => (
                  <Pressable
                    ...
                    onPress={() =>
                      Alert.alert(item.title, "通知详情后续接入。")
                    }
                  >
                    <View
                      style={[
              ...
              {user?.role === "merchant" ? (
                <Pressable
                  style={[
                    styles.trendNoticeRow,
                    { backgroundColor: colors.surface },
                  ]}
                  onPress={() =>
                    navigation.navigate("MerchantTrendNotifications", {
                      entryEdge: "right",
                    })
                  }
                >
                  ...
                  <View style={styles.strangerText}>
                    <Text
                      style={[styles.strangerTitle, { color: colors.text }]}
                    >
                      运营热门手工甲推送
                    </Text>
                    <Text
                      style={[
                        styles.strangerPreview,
                ...
                >
                  <View style={styles.strangerText}>
                    ...
                    <Text
                      ...
                      numberOfLines={1}
                    >
                      {query.data.stranger_bucket.latest_message_preview ||
                        `${query.data.stranger_bucket.thread_count} 个陌生人会话`}
                    </Text>
                  </View>
                  {query.data.stranger_bucket.unread_count ? (
                    <View
                      style={[
                        styles.strangerBadge,
                        { backgroundColor: colors.accent },
                      ]}
                    >
                      <Text style={styles.strangerBadgeText}>
                        {query.data.stranger_bucket.unread_count > 99
                          ? "99+"
                          : query.data.stranger_bucket.unread_count}
                      </Text>
                    </View>
              ...
              <View style={styles.sectionHeaderRow}>
                ...
                >
                  <Text
                    style={[
                      styles.readAllText,
                      { color: unreadCount ? colors.accent : colors.subtext },
                    ]}
                  >
