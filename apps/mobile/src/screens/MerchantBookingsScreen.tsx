...
export function MerchantBookingsScreen({ navigation }: any) {
  ...
  const mutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: Booking["status"] }) => api.updateMerchantBookingStatus(id, status),
    onSuccess: () => void queryClient.invalidateQueries({ queryKey: ["merchant-bookings"] }),
  });

  if (!token || user?.role !== "merchant") {
    return <RequireLogin onLogin={() => navigation.navigate("Login")} message="商家账号登录后查看预约" />;
  }
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {query.data?.items.length ? (
          query.data.items.map((item) => (
            <View key={item.id} style={[styles.card, { backgroundColor: colors.surface }]}>
              ...
              <View style={styles.body}>
                ...
                <Text style={[styles.meta, { color: colors.subtext }]}>门店：{item.shop_name}</Text>
                {item.status === "pending" ? (
                  <View style={styles.actionRow}>
                    <Pressable style={[styles.actionButton, { backgroundColor: colors.accentSoft }]} onPress={() => mutation.mutate({ id: item.id, status: "accepted" })}>
                      <Text style={[styles.actionText, { color: colors.accent }]}>接受</Text>
                    </Pressable>
                    <Pressable style={[styles.actionButton, { backgroundColor: colors.surfaceAlt }]} onPress={() => mutation.mutate({ id: item.id, status: "rejected" })}>
                      <Text style={[styles.actionText, { color: colors.subtext }]}>拒绝</Text>
                    </Pressable>
                  </View>
                ) : item.status === "accepted" ? (
                  <Pressable style={[styles.completeButton, { backgroundColor: colors.accent }]} onPress={() => mutation.mutate({ id: item.id, status: "completed" })}>
          ...
          <View style={[styles.empty, { backgroundColor: colors.surface }]}>
            <Ionicons name="calendar-outline" size={30} color={colors.subtext} />
            <Text style={[styles.emptyText, { color: colors.subtext }]}>暂时还没有预约意向单</Text>
          </View>
        )}
      </ScrollView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  actionButton: { flex: 1, borderRadius: 12, paddingVertical: 8, alignItems: "center" },
  actionText: { fontWeight: "800" },
  completeButton: { marginTop: 4, borderRadius: 12, paddingVertical: 9, alignItems: "center" },
