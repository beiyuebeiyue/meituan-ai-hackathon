...
export function WearableOrderScreen() {
  ...
  const submitDemoOrder = () => {
    Alert.alert("下单成功（演示）", "这是一笔用于产品展示的穿戴甲演示订单，不会写入真实订单。");
  };
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={[styles.addressCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <View style={[styles.addressIcon, { backgroundColor: colors.accentSoft }]}>
            <Ionicons name="location-outline" size={22} color={colors.accent} />
          </View>
          <View style={styles.addressCopy}>
            <Text style={[styles.addressTitle, { color: colors.text }]}>收货信息</Text>
            <Text style={[styles.addressText, { color: colors.subtext }]}>演示地址 · 下单时由用户填写</Text>
          </View>
        </View>
          ...
          {query.isLoading ? (
            ...
            <View style={styles.productRow}>
              ...
              <View style={styles.productCopy}>
                ...
                <Text style={[styles.productMeta, { color: colors.subtext }]}>24片装 · 赠果冻胶 · x1</Text>
              </View>
              <Text style={[styles.productPrice, { color: colors.text }]}>{PRODUCT_PRICE}</Text>
            </View>
          ) : (
      ...
      <View style={[styles.bottomBar, { backgroundColor: colors.surface, borderTopColor: colors.border }]}>
        <View>
          <Text style={[styles.bottomLabel, { color: colors.subtext }]}>合计</Text>
          <Text style={[styles.bottomPrice, { color: colors.text }]}>{PRODUCT_PRICE}</Text>
        </View>
        <PrimaryButton label="提交演示订单" onPress={submitDemoOrder} disabled={!query.data} style={styles.submitButton} />
      </View>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  orderCard: { borderWidth: StyleSheet.hairlineWidth, borderRadius: 24, padding: 15, gap: 14 },
  storeRow: { flexDirection: "row", alignItems: "center", gap: 8 },
  storeName: { fontSize: 16, fontWeight: "900" },
  loadingBlock: { minHeight: 140, alignItems: "center", justifyContent: "center", gap: 10 },
  ...
  typePill: { alignSelf: "flex-start", borderRadius: 999, paddingHorizontal: 9, paddingVertical: 4 },
  ...
  summaryValue: { flexShrink: 1, textAlign: "right", fontSize: 14, fontWeight: "800" },
  bottomBar: {
    position: "absolute",
    left: 0,
    right: 0,
